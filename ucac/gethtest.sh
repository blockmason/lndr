...
rm -rf build
mkdir $GETH_FOLDER && mkdir $GETH_FOLDER/privchain
cat << EOF > $GETH_FOLDER/genesis.json
{
    "config": {
        "chainId": 88888,
        "homesteadBlock": 0,
...
geth --datadir $GETH_FOLDER/privchain init $GETH_FOLDER/genesis.json

geth --port 3001 --networkid 58342 --nodiscover --datadir="$GETH_FOLDER/privchain" --maxpeers=0 \
...
do
    ...
    geth --datadir $GETH_FOLDER/privchain --password <(echo "pass") account import <(echo $i)
done
